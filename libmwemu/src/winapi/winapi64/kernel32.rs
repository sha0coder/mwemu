...
fn CreateThread(emu: &mut emu::Emu) {
    ...
    log::info!("Tid_ptr address mapped: {}", emu.maps.is_mapped(tid_ptr_addr));
    ...
    let flags = match emu.maps.read_qword(flags_addr) {
        ...
        None => {
            panic!("Failed to read flags from 0x{:x} - unmapped memory", flags_addr);
        }
    };
    ...
    let tid_ptr = match emu.maps.read_qword(tid_ptr_addr) {
        ...
        None => {
            panic!("Failed to read tid_ptr from 0x{:x} - unmapped memory", tid_ptr_addr);
        }
    };
    
    // Check if tid_ptr points to valid memory
    if tid_ptr > 0 {
        log::info!("Checking if tid_ptr 0x{:x} is mapped: {}", tid_ptr, emu.maps.is_mapped(tid_ptr));
    ...
    log::info!("Thread start address 0x{:x} mapped: {}", start_addr, emu.maps.is_mapped(start_addr));
    ...
    if stack_sz > 0 {
        if let Some(stack_base) = emu.maps.alloc(stack_sz) {
            ...
            log::info!("Allocated stack: 0x{:x} - 0x{:x}", stack_base, stack_base + stack_sz);
        } else {
    ...
    if tid_ptr > 0 {
        if emu.maps.is_mapped(tid_ptr) {
            ...
            panic!("CANNOT WRITE: tid_ptr 0x{:x} points to unmapped memory!", tid_ptr);
        }
    } else {
    ...
    for (idx, thread) in emu.threads.iter().enumerate() {
        log::info!("  threads[{}]: ID=0x{:x}, suspended={}, RIP=0x{:x}", 
                idx, thread.id, thread.suspended, thread.regs.rip);
    }
...
fn GetCurrentDirectoryW(emu: &mut emu::Emu) {
    ...
    if emu.maps.is_mapped(buff_ptr) && buff_len > path.len() {
        ...
        emu.regs_mut().rax = path.len() as u64;
    } else {
        emu.regs_mut().rax = 0;
    }
