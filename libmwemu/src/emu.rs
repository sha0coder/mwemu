...
impl Emu {
    ...
    pub fn handle_winapi(&mut self, addr: u64) {
        let name = match self.maps.get_addr_name(addr) {
            ...
            None => {
                ...
                self.exception(ExceptionType::RIP_SET_TO_NON_MAPPED);
                return;
            }
        };
        if self.cfg.is_64bits {
    ...
    pub fn exception(&mut self, ex: ExceptionType) {
        let addr: u64;
        let next: u64;

        let handle_exception: bool = match self.hooks.hook_on_exception {
            Some(hook_fn) => hook_fn(self, self.regs.rip, ex),
            None => true,
        };
                    ...
                    value = match sz {
                        64 => match self.maps.read_qword(mem_addr) {
                            ...
                            None => {
                                ...
                                self.exception(ExceptionType::QWORD_DEREFERENCE);
                                return None;
                            }
                        },
                        ...
                        32 => match self.maps.read_dword(mem_addr) {
                            ...
                            None => {
                                ...
                                self.exception(ExceptionType::DWORD_DEREFERENCE);
                                return None;
                            }
                        },
                        ...
                        16 => match self.maps.read_word(mem_addr) {
                            ...
                            None => {
                                ...
                                self.exception(ExceptionType::WORD_DEREFERENCE);
                                return None;
                            }
                        },
                        ...
                        8 => match self.maps.read_byte(mem_addr) {
                            ...
                            None => {
                                ...
                                self.exception(ExceptionType::BYTE_DEREFERENCE);
                                return None;
                            }
                        },
                        ...
                        64 => {
                            if !self.maps.write_qword(mem_addr, value2) {
                                if self.cfg.skip_unimplemented {
                                    ...
                                    self.exception(ExceptionType::BAD_ADDRESS_DEREFERENCE);
                                    return false;
                                }
                            }
                        }
            ...
            OpKind::Memory => {
                    ...
                    .virtual_address(noperand, 0, |reg, idx, _sz| Some(self.regs.get_reg(reg)))
                {
                    ...
                    None => {
                        ...
                        self.exception(ExceptionType::XMM_OPERAND_READ);
                        return None;
                    }
                };
