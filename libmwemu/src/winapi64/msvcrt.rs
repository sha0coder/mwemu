...
fn _initterm(emu: &mut emu::Emu) {
    let start_ptr = emu.regs.rcx;
    let end_ptr = emu.regs.rdx;
...
fn fopen(emu: &mut emu::Emu) {
    let filepath_ptr = emu.regs.rcx;
    let mode_ptr = emu.regs.rdx;
...
fn fwrite(emu: &mut emu::Emu) {
    ...
    let nemb = emu.regs.r8;
    let file = emu.regs.r9;
...
fn strtok(emu: &mut emu::Emu) {
    ...
    let delim_ptr = emu.regs.rdx;

    if str_ptr == 0 || delim_ptr == 0 {
        emu.regs.rax = 0;
        log::info!(
            "{}** {} msvcrt!strtok {}",
            emu.colors.light_red,
