...
pub fn BasepProcessInvalidImage(emu: &mut emu::Emu) {
    let _a1 = emu
        .maps
    ...
    let _a20 = emu
        .maps
        .read_dword(emu.regs().get_esp() + 76)
        .expect("cannot read the api parameter");
    let _a21 = emu
        .maps
        .read_dword(emu.regs().get_esp() + 80)
        .expect("cannot read the api parameter");

    log_red!(emu, "kernel32!BasepProcessInvalidImage");
