...
pub fn CreateFileA(emu: &mut emu::Emu) {
    ...
    log_red!(emu, "** {} kernel32!CreateFileA name = {} {}", emu.pos, name_utf8, emu.colors.nc);
    ...
    match FileHandle::new(
        ...
        Ok(file_handle) => {
            ...
            log_red!(emu, "** {} kernel32!CreateFileA SUCCESS, handle: 0x{:x}", emu.pos, handle_key);
        },
        Err(e) => {
            error!("CreateFileA failed for '{}': {}", name_utf8, e);
            emu.regs_mut().rax = INVALID_HANDLE_VALUE as u64;
            log_red!(emu, "** {} kernel32!CreateFileA FAILED, returning INVALID_HANDLE_VALUE", emu.pos);
        }
    }
}
