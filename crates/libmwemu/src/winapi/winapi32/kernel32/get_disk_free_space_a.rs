...
pub fn GetDiskFreeSpaceA(emu: &mut emu::Emu) {
    ...
    let sectors_per_cluster_ptr = emu
        ...
        .expect("kernel32!GetDiskFreeSpaceA cannot read sectors_per_cluster_ptr") as u64;
    let bytes_per_sector_ptr = emu
        .maps
        .read_dword(emu.regs().get_esp() + 8)
        .expect("kernel32!GetDiskFreeSpaceA cannot read bytes_per_sector_ptr") as u64;
    let free_clusters_ptr = emu
        .maps
    ...
    log_red!(
        emu,
        "kernel32!GetDiskFreeSpaceA path:`{}`",
        root_path
    );
