...
fn test_add_rsp_cleanup_params() {
    ...
    assert_eq!(
        emu.regs().rsp,
        0x1000,
        "Stack cleaned up after parameter push"
    );
}
...
fn test_mov_rsp_rbp() {
    ...
    assert_eq!(
        emu.regs().rsp,
        0x2000,
        "RSP set from RBP (like LEAVE part 1)"
    );
}
...
fn test_mov_rbp_rsp() {
    ...
    assert_eq!(
        emu.regs().rbp,
        0x1000,
        "RBP gets RSP value (prologue pattern)"
    );
}
...
fn test_rsp_wraparound_add() {
    ...
    emu.maps
        .create_map(
            "stack_test",
            0xFFFFFFFFFFFFFFF0 - (0xFFFFFFFFFFFFFFF0 / 2),
            0x1000,
            crate::maps::mem64::Permission::READ_WRITE_EXECUTE,
        )
        .unwrap();
