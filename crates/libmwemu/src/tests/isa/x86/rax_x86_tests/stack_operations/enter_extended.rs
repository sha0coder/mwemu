...
fn test_enter_basic_no_nesting() {
    ...
    assert_eq!(
        emu.regs().rsp,
        0x1000 - 8,
        "RSP decremented by 8 (pushed RBP)"
    );
...
fn test_enter_leave_nested() {
    ...
    assert_eq!(
        emu.regs().rsp,
        0x1000,
        "RSP restored after nested ENTER/LEAVE"
    );
    assert_eq!(
        emu.regs().rbp,
        0x2000,
        "RBP restored after nested ENTER/LEAVE"
    );
}
