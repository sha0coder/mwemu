...
use crate::maps::mem64::Permission; // Assuming crate root has winapi module public or we can access it. 
...
fn test_write_file() {
    ...
    emu.maps.create_map("buffer", buff_addr, 0x1000, Permission::READ_WRITE);
    ...
    emu.maps.create_map("written", written_ptr, 0x1000, Permission::READ_WRITE);
...
fn test_get_module_handle_64() {
    ...
    emu.maps.create_map("data", name_addr, 0x1000, Permission::READ_WRITE);
    emu.maps.write_string(name_addr, "kernel32.dll");
    
    // Create the expected module map "kernel32.pe"
    emu.maps.create_map("kernel32.pe", 0x7FF10000000, 0x10000, Permission::READ_EXECUTE);
    ...
    assert_eq!(h_mod, 0x7FF10000000, "GetModuleHandleA('kernel32.dll') returned incorrect base");
}
