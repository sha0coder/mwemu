...
fn test_pushad_decrements_stack_pointer() {
    ...
    assert_eq!(emu.regs().rsp, 0x8000 - 32, "RSP should be decremented by 32");
}
...
fn test_pusha_popa_roundtrip() {
    ...
    assert_eq!(emu.regs().rax & 0xFFFFFFFF, 0xAAAAAAAA, "RAX restored after POPAD");
    assert_eq!(emu.regs().rbx & 0xFFFFFFFF, 0xBBBBBBBB, "RBX restored after POPAD");
}
...
fn test_pushad_popa_with_alternating_modification() {
    ...
    assert_eq!(emu.regs().rcx & 0xFFFFFFFF, 0x11111111, "ECX contains restored value");
}
...
fn test_popad_overwrites_current_values() {
    ...
    assert_eq!(emu.regs().rax & 0xFFFFFFFF, 0x11111111, "POPAD overwrites EAX");
    assert_eq!(emu.regs().rbx & 0xFFFFFFFF, 0x22222222, "POPAD overwrites EBX");
}
...
fn test_pushad_popa_preserves_higher_bits_in_64bit() {
    ...
    assert_eq!(emu.regs().rax & 0xFFFFFFFF, 0x11111111, "Lower 32 bits unchanged");
    assert_eq!(emu.regs().rax >> 32, 0x11111111u64, "Upper 32 bits unchanged");
}
...
fn test_pushad_does_not_fault_with_various_rsp() {
    ...
    emu.run(None).unwrap();
        ...
        assert_eq!(emu.regs().rsp, rsp_val - 32, "RSP decremented correctly for RSP={:x}", rsp_val);
    }
}
