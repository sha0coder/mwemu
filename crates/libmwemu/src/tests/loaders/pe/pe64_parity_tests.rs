...
fn run_parity_test(fixture: &str, test_name: &str, check: impl Fn(&PE64, &LiefPe) -> Vec<String>) {
    ...
    if !mismatches.is_empty() {
        ...
        panic!("[{}] {} mismatches for {}", test_name, mismatches.len(), fixture);
    }
}
...
fn parity_serialization_lief_raw_bytes() {
    ...
    assert_eq!(reloaded.image_base(), image_base, "image_base mismatch after raw reload");
    assert_eq!(reloaded.entry_point(), entry_point, "entry_point mismatch after raw reload");
    assert_eq!(reloaded.num_sections(), num_sections, "section count mismatch after raw reload");
}
...
fn parity_serialization_backward_compat_lowercase() {
    ...
    let parsed: SerializablePe64Backend = serde_yaml::from_str(yaml_legacy)
        .expect("should deserialize lowercase 'legacy'");
    assert_eq!(parsed, SerializablePe64Backend::Legacy);

    let yaml_lief = "\"lief\"";
    let parsed: SerializablePe64Backend = serde_yaml::from_str(yaml_lief)
        .expect("should deserialize lowercase 'lief'");
...
fn compare_headers(legacy: &PE64, lief: &LiefPe) -> Vec<String> {
    ...
    if legacy.opt.image_base != lief.image_base() {
        m.push(format!("image_base: legacy=0x{:x} lief=0x{:x}", legacy.opt.image_base, lief.image_base()));
    }
    if legacy.opt.address_of_entry_point as u64 != lief.entry_point() {
        m.push(format!("entry_point: legacy=0x{:x} lief=0x{:x}", legacy.opt.address_of_entry_point, lief.entry_point()));
    }
    if legacy.opt.size_of_image != lief.virtual_size() as u32 {
        m.push(format!("size_of_image: legacy=0x{:x} lief=0x{:x}", legacy.opt.size_of_image, lief.virtual_size()));
    }
    if legacy.opt.size_of_headers != lief.size_of_headers() {
        m.push(format!("size_of_headers: legacy=0x{:x} lief=0x{:x}", legacy.opt.size_of_headers, lief.size_of_headers()));
    }
    if legacy.opt.section_alignment != lief.section_alignment() {
        m.push(format!("section_alignment: legacy=0x{:x} lief=0x{:x}", legacy.opt.section_alignment, lief.section_alignment()));
    }
    if legacy.dos.e_lfanew != lief.e_lfanew() {
        m.push(format!("e_lfanew: legacy=0x{:x} lief=0x{:x}", legacy.dos.e_lfanew, lief.e_lfanew()));
    }
...
fn compare_sections(legacy: &PE64, lief: &LiefPe) -> Vec<String> {
    ...
    if legacy_count != lief_count {
        m.push(format!("section count: legacy={} lief={}", legacy_count, lief_count));
        return m;
    }
    ...
    for i in 0..legacy_count {
        ...
        if legacy_name != lief_name {
            m.push(format!("section {} name: legacy='{}' lief='{}'", i, legacy_name, lief_name));
        }
        if ls.virtual_address != lief_sect.virtual_address() as u32 {
            m.push(format!("section {} VA: legacy=0x{:x} lief=0x{:x}", i, ls.virtual_address, lief_sect.virtual_address()));
        }
        if ls.virtual_size != lief_sect.virtual_size() {
            m.push(format!("section {} virtual_size: legacy=0x{:x} lief=0x{:x}", i, ls.virtual_size, lief_sect.virtual_size()));
        }
        if ls.size_of_raw_data != lief_sect.sizeof_raw_data() {
            m.push(format!("section {} raw_size: legacy=0x{:x} lief=0x{:x}", i, ls.size_of_raw_data, lief_sect.sizeof_raw_data()));
        }
        if ls.characteristics != lief_sect.characteristics().bits() as u32 {
            m.push(format!("section {} characteristics: legacy=0x{:x} lief=0x{:x}", i, ls.characteristics, lief_sect.characteristics().bits()));
        }
        ...
        if legacy_hash != lief_hash {
            m.push(format!("section {} raw_byte_hash: legacy={:016x} lief={:016x}", i, legacy_hash, lief_hash));
        }
    }
...
fn compare_imports(legacy: &PE64, lief: &LiefPe) -> Vec<String> {
    ...
    let legacy_dlls: Vec<_> = legacy.image_import_descriptor.iter()
    ...
    if legacy_dlls.len() != lief_imports.len() {
        m.push(format!("import DLL count: legacy={} lief={}", legacy_dlls.len(), lief_imports.len()));
    }

    for ld in &legacy_dlls {
        if !lief_imports.iter().any(|li| li.dll_name.eq_ignore_ascii_case(&ld.name)) {
            m.push(format!("DLL '{}' in legacy but not in LIEF", ld.name));
        }
    }

    for li in &lief_imports {
        if !legacy_dlls.iter().any(|ld| ld.name.eq_ignore_ascii_case(&li.dll_name)) {
            m.push(format!("DLL '{}' in LIEF but not in legacy", li.dll_name));
            continue;
        }

        let matching_legacy = legacy_dlls.iter()
        ...
        let mut off_addr = PE64::vaddr_to_off(&legacy.sect_hdr, matching_legacy.first_thunk) as usize;
        ...
        loop {
            ...
            let thunk_data = u64::from_le_bytes(legacy.raw[off_name..off_name+8].try_into().unwrap_or([0;8]));
            if thunk_data == 0 {
                break;
            }
        ...
        let lief_func_names: Vec<String> = li.functions.iter()
            .filter(|lf| lf.rva != 0)
            .map(|lf| {
                if lf.name.is_empty() {
        ...
        if legacy_func_names.len() != lief_func_names.len() {
            m.push(format!(
                ...
                li.dll_name, legacy_func_names.len(), lief_func_names.len()
            ));
        }

        for lfn in &legacy_func_names {
            if !lief_func_names.iter().any(|lif| lif.eq_ignore_ascii_case(lfn)) {
                m.push(format!(
        ...
        for lif in &lief_func_names {
            if !legacy_func_names.iter().any(|lfn| lfn.eq_ignore_ascii_case(lif)) {
                m.push(format!(
...
fn compare_relocations(legacy: &PE64, lief: &LiefPe) -> Vec<String> {
    ...
    match lief_reloc_dir {
        Some(ld) => {
            if legacy_reloc_dir.virtual_address != ld.rva() as u32 {
                m.push(format!("reloc dir RVA: legacy=0x{:x} lief=0x{:x}", legacy_reloc_dir.virtual_address, ld.rva()));
            }
            if legacy_reloc_dir.size != ld.size() as u32 {
                m.push(format!("reloc dir size: legacy=0x{:x} lief=0x{:x}", legacy_reloc_dir.size, ld.size()));
            }
        }
        None => {
    ...
    while off < end_off && off + 8 <= legacy.raw.len() {
        ...
        let block_sz = u32::from_le_bytes(legacy.raw[off+4..off+8].try_into().unwrap_or([0;4]));
        if page_va == 0 && block_sz == 0 { break; }
        if block_sz < 8 { break; }
        let entries = (block_sz - 8) / 2;
        let mut entry_off = off + 8;
        for _ in 0..entries {
            if entry_off + 2 > legacy.raw.len() { break; }
            let entry = u16::from_le_bytes(legacy.raw[entry_off..entry_off+2].try_into().unwrap_or([0;2]));
    ...
    let lief_reloc_pairs: Vec<(u64, u16)> = lief_relocs.iter().map(|r| (r.rva, r.reloc_type)).collect();

    if legacy_reloc_pairs.len() != lief_reloc_pairs.len() {
        m.push(format!("reloc entry count: legacy={} lief={}", legacy_reloc_pairs.len(), lief_reloc_pairs.len()));
    }
    ...
    for &(lrva, ltype) in &legacy_sorted {
        if !lief_sorted.contains(&(lrva, ltype)) {
            m.push(format!("reloc (RVA=0x{:x} type={}) in legacy but not in LIEF", lrva, ltype));
            if m.len() > 10 { break; }
        }
    }
    for &(lrva, ltype) in &lief_sorted {
        if !legacy_sorted.contains(&(lrva, ltype)) {
            m.push(format!("reloc (RVA=0x{:x} type={}) in LIEF but not in legacy", lrva, ltype));
            if m.len() > 10 { break; }
        }
    }
...
fn compare_resources(legacy: &PE64, lief: &LiefPe) -> Vec<String> {
    ...
    if legacy_rsrc_dir.virtual_address != 0 && lief_entries.is_empty() {
        m.push(format!("legacy has resource dir (RVA=0x{:x}) but LIEF returned zero entries", legacy_rsrc_dir.virtual_address));
    }

    if legacy_rsrc_dir.virtual_address == 0 && !lief_entries.is_empty() {
        m.push(format!("legacy has no resource dir but LIEF returned {} entries", lief_entries.len()));
    }

    for entry in &lief_entries {
        if entry.size > 0 && entry.data_rva == 0 {
            m.push(format!("resource entry type={:?} name={:?} has size {} but data_rva is 0", entry.type_id, entry.name, entry.size));
        }
    }
    ...
    if !lief_entries.is_empty() && legacy_rsrc_dir.virtual_address != 0 {
        ...
        if total_lief_size == 0 && !lief_entries.is_empty() {
            m.push(format!("LIEF has {} resource entries but zero total content bytes", lief_entries.len()));
        }
    }
    ...
    if !lief_entries.is_empty() && legacy_rsrc_dir.virtual_address != 0 {
        for le in &lief_entries {
            if le.size > 0 && le.data_rva == 0 {
                m.push(format!("resource entry type={:?} name={:?} has size {} but data_rva is 0", le.type_id, le.name, le.size));
            }
        }
    }
...
fn compare_delay_loads(legacy: &PE64, lief: &LiefPe) -> Vec<String> {
    ...
    if !has_legacy_delay && !lief_descriptors.is_empty() {
        m.push(format!("legacy has no delay-load dir but LIEF returned {} descriptors", lief_descriptors.len()));
    }
    ...
    if legacy_desc.len() != lief_descriptors.len() {
        m.push(format!("delay-load descriptor count: legacy={} lief={}", legacy_desc.len(), lief_descriptors.len()));
    }
    ...
    for i in 0..count {
        ...
        if !ld.dll_name.is_empty() {
            ...
            if !leg_dll_name.is_empty() && ld.dll_name.to_lowercase() != leg_dll_name.to_lowercase() {
                m.push(format!("Descriptor {} dll_name mismatch: legacy='{}' lief='{}'", i, leg_dll_name, ld.dll_name));
            }
        }
        if ld.attributes != legacy_desc[i].attributes {
            m.push(format!("Descriptor {} attributes mismatch: legacy=0x{:x} lief=0x{:x}", i, legacy_desc[i].attributes, ld.attributes));
        }
        if ld.delay_iat != legacy_desc[i].delay_iat {
            m.push(format!("Descriptor {} delay_iat mismatch: legacy=0x{:x} lief=0x{:x}", i, legacy_desc[i].delay_iat, ld.delay_iat));
        }
        if ld.delay_int != legacy_desc[i].delay_int {
            m.push(format!("Descriptor {} delay_int mismatch: legacy=0x{:x} lief=0x{:x}", i, legacy_desc[i].delay_int, ld.delay_int));
        }
    }
