...
fn test_virtual_alloc_32() {
    ...
    emu.maps.create_map("stack", 0x0, 0x20000, Permission::READ_WRITE);
...
fn test_write_file_32() {
    ...
    let buf_addr = 0x20000;
    emu.maps.create_map("buffer", buf_addr, 0x2000, Permission::READ_WRITE); // Size 0x2000 covers 0x21000
    ...
    // Check bytes written
    let bytes = emu.maps.read_dword(written_ptr).expect("Cannot read bytes written");
    assert_eq!(bytes, 100);
}
